...
export default function ArticleDetail({ articleData, idx }: { articleData: NaverItemData | null; idx: string }) {
  ...
  const [formData, setFormData] = useState({
    ...
    permalink: `${process.env.NEXT_PUBLIC_API_URL}/article-memorial/${idx}`,
    idx: idx,
  ...
  const fetchNaverCommentData = async () => {
    try {
      const response = await fetch(`/api/comments?collection=naver-memorial&idx=${idx}`);
...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  if (articleId && typeof articleId === 'string') {
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/articleNews?id=${articleId.substring(14)}`);
